...
- open local Ubuntu firewall TCP `22` and `8000` or the chosen app/API port publicly
- allow SMB `445` and NFSv4 `2049` only from the configured private share CIDR
...
./install.sh --public-host migrator.example.com --share-allow-cidr 10.0.0.0/16
...
`--no-open-firewall` skips opening local Ubuntu firewall ports during install. By default TCP `22` and the app/API port are opened publicly, while SMB `445` and NFSv4 `2049` only allow `10.0.0.0/8`. Use `--share-allow-cidr` for a narrower VCN subnet, a specific instance such as `10.0.1.121/32`, or a comma-separated list.
After installation, timezone and NTP servers can also be changed from `Settings` -> `Time & NTP`.

## Optional SMB/NFS Sharing

No SMB or NFS share is enabled during installation. The installer prepares private-CIDR firewall rules and places a root-owned helper plus a sudoers rule so the web UI can enable a managed share later.
...
- ensure inbound TCP `445` is allowed only from the configured private share CIDR and save the firewall rule when supported
...
- ensure inbound TCP `2049` is allowed only from the configured private share CIDR and save the firewall rule when supported
