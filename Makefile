...
help:
	...
	@printf '  PUBLIC_HOST=<server-ip-or-dns> API_PORT=8000 FRONTEND_PORT=5173 SERVICE_PREFIX=migrator\n'

install:
	./install.sh --api-port "$(API_PORT)" --frontend-port "$(FRONTEND_PORT)" --service-prefix "$(SERVICE_PREFIX)" $(if $(PUBLIC_HOST),--public-host "$(PUBLIC_HOST)")

install-print-token:
	./install.sh --api-port "$(API_PORT)" --frontend-port "$(FRONTEND_PORT)" --service-prefix "$(SERVICE_PREFIX)" --print-token $(if $(PUBLIC_HOST),--public-host "$(PUBLIC_HOST)")
